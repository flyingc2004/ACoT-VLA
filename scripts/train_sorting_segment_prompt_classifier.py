#!/usr/bin/env python3
"""Train a classifier for sorting segment-specific prompt switching.

This script is intentionally separate from train_sorting_phase_classifier.py.
It expects seven segment-stage classes built by build_sorting_segment_prompt_dataset.py.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train sorting segment prompt classifier")
    parser.add_argument(
        "--data-dir",
    ...
    parser.add_argument(
        "--output-dir",
        default="outputs/sorting_segment_prompt_classifier",
    ...
    parser.add_argument("--enforce-target-accuracy", action="store_true")
    parser.add_argument("--save-name", default="sorting_segment_prompt_classifier_best.pt")
...
def evaluate(
    ...
    with torch.no_grad():
        for images, targets in loader:
            ...
            for target, pred in zip(targets.detach().cpu().numpy(), preds.detach().cpu().numpy(), strict=False):
...
def main() -> None:
    ...
    train_transform = tv_transforms.Compose(
        [
            ...
            tv_transforms.ColorJitter(brightness=0.15, contrast=0.20, saturation=0.10, hue=0.0),
    ...
    if set(class_names) != EXPECTED_LABELS:
        raise ValueError(f"Dataset classes differ from expected labels: found={class_names}, expected={sorted(EXPECTED_LABELS)}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        val_loss, val_acc, confusion = evaluate(model, val_loader, criterion, device, len(class_names))
    ...
    metrics = {
        ...
        "history": history,
        "class_names": class_names,
        "best_confusion": best_confusion.tolist() if best_confusion is not None else None,
    }

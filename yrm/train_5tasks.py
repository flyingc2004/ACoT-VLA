...
def _repo_paths() -> list[str]:
    ...
    base = "/data/Dataset/Reasoning2Action-Sim/dataset_without_depth"
    return [
        f"{base}/pour_workpiece",
        f"{base}/open_door",
        f"{base}/take_wrong_item_shelf",
        f"{base}/scoop_popcorn",
        f"{base}/scoop_popcorn_part_2",
        f"{base}/hold_pot",
    ]
...
def build_config(*, exp_name: str, overwrite: bool, resume: bool, batch_size: int, num_workers: int):
    ...
    baseline_params = os.environ.get("BASELINE_PARAMS", "/data/checkpoints/params")
    baseline_assets = os.environ.get("BASELINE_NORM_ASSETS_DIR", "/data/checkpoints/assets")
